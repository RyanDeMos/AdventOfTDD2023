...
func main() {
	...
	for scanner.Scan() {
		...
		if (colourCounter["green"] <= maxGreen) && (colourCounter["blue"] <= maxBlue) && (colourCounter["red"] <= maxRed) {
			ValidGameIdSum += getGameID(parsedLine[0])
		}
	}
	fmt.Printf("Total valid game ids are: %d", ValidGameIdSum)
}
