...
func main() {
	// fileLines := ReadFileIntoStringSlice("./Day9/Part2/input/testInput.txt")
...
func findNextInSequence(line string) int {
	...
	diffNotAllZero := true
	firstInSequence := sequence[0]
	parityTracker := 1
	for diffNotAllZero {
		sequence, diffNotAllZero = nextDiff(sequence)
		firstInSequence += sequence[0] * int(math.Pow(-1, float64(parityTracker)))
		parityTracker += 1
	}
